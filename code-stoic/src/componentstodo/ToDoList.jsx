...
export default function ToDoList({todos, setTodos}) {
    ...
    const sortTodo = todos.slice()
                          .sort((a,b) => a.done - b.done)   
    // slice method create a copy of todos and sorts the values
    // We are clasting the boolean to number to sort accending to decending sorting.
    ...
    return <div className={list.list}>
        ...
        {sortTodo.map((e) => ( // <h4>{e}</h4> // This is pass value directly inside the html tag
            <ToDoItem key={e.name} item={e} todos={todos} setTodos={setTodos}/> // This is to pass value to the component
        ))}
    </div>
}